...
do
  if [ ${cam} != "CAMERA" ] && ( [ "$4" = "-noskip" ] || [ "${skip}" = "" ] ); then
    ...
    if [ "$4" = "-noskip" ] && [ ${srcfw} != ${fw} ]; then jmp=1; else jmp=0; fi
    ...
    if ( [ "${ist2}" = "" ] || [ "$4" != "-not2" ] ) && [ ${jmp} -eq 0 ] ; then
      $1 -s --no-print-directory TARGET_FW=${fw} PLATFORM=${cam} PLATFORMSUB=${srcfw} STATE=${state} SKIP_AUTOBUILD=${skip} ${2}${cpy} || exit 1
    fi
  fi
done < $3
