...
void build_shot_histogram()
{
 ...
 marginstep= (camera_info.raw_rowpix - 2 * SHOT_HISTOGRAM_MARGIN)/10;

 // In future, support definition of a sort of "spot metering" 
 x0 = SHOT_HISTOGRAM_MARGIN + shot_margin_left * marginstep;
 x1 = camera_info.raw_rowpix - SHOT_HISTOGRAM_MARGIN - shot_margin_right * marginstep;
 y0 = SHOT_HISTOGRAM_MARGIN + shot_margin_top * marginstep;
 y1 = camera_info.raw_rows - SHOT_HISTOGRAM_MARGIN - shot_margin_bottom * marginstep;

 //x0 = SHOT_HISTOGRAM_MARGIN ;
 //x1 = camera_info.raw_rowpix - SHOT_HISTOGRAM_MARGIN ;
 //y0 = SHOT_HISTOGRAM_MARGIN;
 //y1 = camera_info.raw_rows - SHOT_HISTOGRAM_MARGIN;
