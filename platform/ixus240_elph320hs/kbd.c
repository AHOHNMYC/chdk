...
//    if(print_key_index != -1 && !draw_test_pixel(keymap[print_key_index].x1+1, keymap[print_key_index].y1+1, (guiMode)?COLOR_GREEN:COLOR_WHITE)) redraw_buttons = 1;
    ...
    if (redraw_buttons)
    {
        ...
        twoColors c1 = MAKE_COLOR((camMode&MODE_VID)?COLOR_TRANSPARENT:COLOR_BLACK, COLOR_WHITE);
        twoColors c2 = MAKE_COLOR((camMode&MODE_VID)?COLOR_TRANSPARENT:COLOR_BLACK, COLOR_GREEN);
        ...
        for (i=0; keymap[i].hackkey; i++)
        {
            if (is_button_displayed(i, guiMode, camMode) && keymap[i].nm)
            {
                ...
                {
                    // unclear what was supposed to be displayed here
                    // use hard coded string rather than uninitialized buffer
//                    draw_string(x1+4, y1+ofst, buf, cl);
                    draw_string(x1+4, y1+ofst, "UD!", cl);
                }
                else
                {
