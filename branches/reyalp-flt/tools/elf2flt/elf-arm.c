...
//      case R_ARM_JUMP24:		// not needed
			  ...
			  if ( strcmp(symname,"__div0") ) {
					PRINTERR( stderr, "%s point not to the __div0. Something unusual.\n",reloc_name);
              ...
              offset += /*tgt_fulloffset @tsv*/ (text.flat_offset + offs_div0_from_arm) - locoffset;
		 ...
		 )
{
  ...
  switch(type) {
    case R_ARM_ABS32:
    	{
            *(uint32_t*)loc = importidx;

        	if ( FLAG_DUMP_RELOC )
				printf( "R_ARM_ABS32: %p(%s+0x%x=%x): import_value=%d (sym:%s) %d\n", 
					(char*)base_sect->base_addr+rela->r_offset,  base_sect->name, rela->r_offset, locoffset,
					 importidx, symname, flat->import_count
						);
      		*flat_import_cur = locoffset;
      		flat_import_cur++;
            flat->import_count++;
    	}
      	break;
