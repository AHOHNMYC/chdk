...
static int imath_sinr (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(sinr(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_cosr (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(cosr(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_tanr (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(tanr(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_recr (lua_State *L) {
    fixed px, py;
    recr(INT2FIXEDR(luaL_checknumber(L, 1)),
        INT2FIXEDR(luaL_checknumber(L, 2)), &px, &py);
...
static int imath_asinr (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(asinr(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_acosr (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(acosr(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_atanr (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(atanr(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_polr (lua_State *L) {
    fixed r, theta;
    polr(INT2FIXEDR(luaL_checknumber(L, 1)),
        INT2FIXEDR(luaL_checknumber(L, 2)), &r, &theta);
...
static int imath_sind (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(sind(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_cosd (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(cosd(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_tand (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(tand(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_recd (lua_State *L) {
    fixed px, py;
    recd(INT2FIXEDR(luaL_checknumber(L, 1)),
        INT2FIXEDR(luaL_checknumber(L, 2)), &px, &py);
    lua_pushnumber(L, FIXED2INTR(px));
    lua_pushnumber(L, FIXED2INTR(py));
    return 2;
...
static int imath_asind (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(asind(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_acosd (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(acosd(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_atand (lua_State *L) {
    lua_pushnumber(L, FIXED2INTR(atand(INT2FIXEDR(luaL_checknumber(L, 1)))));
    return 1;
}

static int imath_pold (lua_State *L) {
    fixed r, theta;
    pold(INT2FIXEDR(luaL_checknumber(L, 1)),
        INT2FIXEDR(luaL_checknumber(L, 2)), &r, &theta);
    lua_pushnumber(L, FIXED2INTR(r));
    lua_pushnumber(L, FIXED2INTR(theta));
    return 2;
