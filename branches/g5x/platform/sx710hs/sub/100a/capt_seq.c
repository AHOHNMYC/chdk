#include "lolevel.h"
#include "platform.h"
#include "core.h"

// debug
#define CAPTSEQ_DEBUG_LOG 1
extern void _LogCameraEvent(int id,const char *fmt,...);

#define USE_STUBS_NRFLAG 1
#define NR_AUTO (-1) // default value if NRTBL.SetDarkSubType not used is -1 (0 probalby works the same), set to enable auto

#ifdef CAPTSEQ_DEBUG_LOG
//extern int active_raw_buffer;

extern char *hook_raw_image_addr(void);

void log_capt_seq(int m)
{
    _LogCameraEvent(0x60,"cs m:%d rb:0x%08x i:%04d",
                    m,
                    hook_raw_image_addr(),
                    get_exposure_counter());
}
void log_capt_seq2(int m)
{
    _LogCameraEvent(0x60,"cs end m:%d rb:0x%08x i:%04d",
                    m,
                    hook_raw_image_addr(),
                    get_exposure_counter());
}
void log_capt_seq_override(void)
{
    _LogCameraEvent(0x60,"cs override rb:0x%08x i:%04d",
                    hook_raw_image_addr(),
                    get_exposure_counter());
}
#endif

#include "../../../generic/capt_seq.c"

// first paramter matches active_raw_buffer
// second is pointer to structure
extern int _captseq_raw_addr_init(int raw_index, char **ptr);
char *current_raw_addr;

void captseq_raw_addr_init_my(int raw_index,char **ptr) {
    _captseq_raw_addr_init(raw_index,ptr);
    current_raw_addr=*(ptr + 0x5c/4); // @0xfc082212, ptr+0x5c
#ifdef CAPTSEQ_DEBUG_LOG
    _LogCameraEvent(0x60,"rawinit i:0x%x p:0x%x v:0x%x",raw_index,ptr,current_raw_addr);
#endif
}

void clear_current_raw_addr(void) {
    current_raw_addr=NULL;
}

// -f=chdk -s=task_CaptSeq -c=172
// task_CaptSeq 0xfc0646b3
void __attribute__((naked,noinline)) capt_seq_task() {
    asm volatile (
"    push    {r3, r4, r5, r6, r7, lr}\n"
"    ldr     r4, =0x00039e5c\n"
"    movs    r6, #0\n"
"    ldr     r5, =0x0000ba40\n"
"loc_fc0646ba:\n"
"    movs    r2, #0\n"
"    mov     r1, sp\n"
"    ldr     r0, [r5, #8]\n"
"    blx     sub_fc2c7ca0\n" // j_ReceiveMessageQueue
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc0646dc\n"
"    movw    r2, #0x44e\n"
"    ldr     r1, =0xfc064364\n" //  *"SsShootTask.c"
"    movs    r0, #0\n"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"    blx     sub_fc2c7cf8\n" // -> ExitTask
"    pop     {r3, r4, r5, r6, r7, pc}\n"
"loc_fc0646dc:\n"
"    ldr     r0, [sp]\n"
"    ldr     r0, [r0]\n"
"    cmp     r0, #1\n"
"    beq     loc_fc0646f4\n"
"    cmp     r0, #0x29\n"
"    beq     loc_fc0646f4\n"
"    cmp     r0, #0x1e\n"
"    beq     loc_fc0646f4\n"
"    cmp     r0, #0x21\n"
"    beq     loc_fc0646f4\n"
"    bl      sub_fc080fbc\n"
"loc_fc0646f4:\n"
#ifdef CAPTSEQ_DEBUG_LOG
// debug message
"ldr     r0, [sp]\n"
"ldr     r0, [r0]\n"
"bl log_capt_seq\n"
#endif
"    ldr     r0, [sp]\n"
"    ldr     r1, [r0]\n"
"    cmp     r1, #0x2b\n"
"    bhs     loc_fc0647b6\n"
"    tbb     [pc, r1]\n" // (jumptable r1 43 elements)
"branchtable_fc064700:\n"
"    .byte((loc_fc06472c - branchtable_fc064700) / 2)\n" // (case 0)
"    .byte((loc_fc064742 - branchtable_fc064700) / 2)\n" // (case 1)
"    .byte((loc_fc06474a - branchtable_fc064700) / 2)\n" // (case 2)
"    .byte((loc_fc064758 - branchtable_fc064700) / 2)\n" // (case 3)
"    .byte((loc_fc064752 - branchtable_fc064700) / 2)\n" // (case 4)
"    .byte((loc_fc064762 - branchtable_fc064700) / 2)\n" // (case 5)
"    .byte((loc_fc064768 - branchtable_fc064700) / 2)\n" // (case 6)
"    .byte((loc_fc06476e - branchtable_fc064700) / 2)\n" // (case 7)
"    .byte((loc_fc064776 - branchtable_fc064700) / 2)\n" // (case 8)
"    .byte((loc_fc0647a8 - branchtable_fc064700) / 2)\n" // (case 9)
"    .byte((loc_fc064780 - branchtable_fc064700) / 2)\n" // (case 10)
"    .byte((loc_fc064788 - branchtable_fc064700) / 2)\n" // (case 11)
"    .byte((loc_fc06478e - branchtable_fc064700) / 2)\n" // (case 12)
"    .byte((loc_fc064796 - branchtable_fc064700) / 2)\n" // (case 13)
"    .byte((loc_fc06479c - branchtable_fc064700) / 2)\n" // (case 14)
"    .byte((loc_fc0647a2 - branchtable_fc064700) / 2)\n" // (case 15)
"    .byte((loc_fc0647b0 - branchtable_fc064700) / 2)\n" // (case 16)
"    .byte((loc_fc0647c4 - branchtable_fc064700) / 2)\n" // (case 17)
"    .byte((loc_fc0647ca - branchtable_fc064700) / 2)\n" // (case 18)
"    .byte((loc_fc0647d0 - branchtable_fc064700) / 2)\n" // (case 19)
"    .byte((loc_fc0647d6 - branchtable_fc064700) / 2)\n" // (case 20)
"    .byte((loc_fc0647dc - branchtable_fc064700) / 2)\n" // (case 21)
"    .byte((loc_fc0647e0 - branchtable_fc064700) / 2)\n" // (case 22)
"    .byte((loc_fc0647e6 - branchtable_fc064700) / 2)\n" // (case 23)
"    .byte((loc_fc0647ec - branchtable_fc064700) / 2)\n" // (case 24)
"    .byte((loc_fc0647f2 - branchtable_fc064700) / 2)\n" // (case 25)
"    .byte((loc_fc0647f8 - branchtable_fc064700) / 2)\n" // (case 26)
"    .byte((loc_fc0647fe - branchtable_fc064700) / 2)\n" // (case 27)
"    .byte((loc_fc064806 - branchtable_fc064700) / 2)\n" // (case 28)
"    .byte((loc_fc06480c - branchtable_fc064700) / 2)\n" // (case 29)
"    .byte((loc_fc064810 - branchtable_fc064700) / 2)\n" // (case 30)
"    .byte((loc_fc064818 - branchtable_fc064700) / 2)\n" // (case 31)
"    .byte((loc_fc06481e - branchtable_fc064700) / 2)\n" // (case 32)
"    .byte((loc_fc064824 - branchtable_fc064700) / 2)\n" // (case 33)
"    .byte((loc_fc06482a - branchtable_fc064700) / 2)\n" // (case 34)
"    .byte((loc_fc064830 - branchtable_fc064700) / 2)\n" // (case 35)
"    .byte((loc_fc064836 - branchtable_fc064700) / 2)\n" // (case 36)
"    .byte((loc_fc06483c - branchtable_fc064700) / 2)\n" // (case 37)
"    .byte((loc_fc064844 - branchtable_fc064700) / 2)\n" // (case 38)
"    .byte((loc_fc06484a - branchtable_fc064700) / 2)\n" // (case 39)
"    .byte((loc_fc06486e - branchtable_fc064700) / 2)\n" // (case 40)
"    .byte((loc_fc064874 - branchtable_fc064700) / 2)\n" // (case 41)
"    .byte((loc_fc06488e - branchtable_fc064700) / 2)\n" // (case 42)
".align 1\n"
"loc_fc06472c:\n"  // case 0: preshoot, quick press shoot
"    ldr     r0, [r0, #0xc]\n"
"    bl      sub_fc073cbc\n"
#ifdef CAPTSEQ_DEBUG_LOG
"bl log_capt_seq_override\n"
#endif
"    BL      clear_current_raw_addr\n" // +
"    BL      shooting_expo_param_override\n" // +
"    bl      sub_fc079112\n"
"    ldr     r0, [r4, #0x28]\n"
"    cmp     r0, #0\n"
"    beq     loc_fc064740\n"
"    bl      sub_fc07b6ba_my\n" // quick press
"loc_fc064740:\n"
"    b       loc_fc06488e\n"
"loc_fc064742:\n" // case 1: normal shoot
"    ldr     r0, [r0, #0x10]\n"
"    bl      sub_fc07b4ee_my\n" // regular shoot
"    b       loc_fc06488e\n"
"loc_fc06474a:\n"
"    movs    r0, #1\n"
"    bl      sub_fc073f74\n"
"    b       loc_fc06488e\n"
"loc_fc064752:\n"
"    bl      sub_fc073a32\n"
"    b       loc_fc06475e\n"
"loc_fc064758:\n"
"    ldr     r0, [r0, #0xc]\n"
"    bl      sub_fc073ca4\n"
"loc_fc06475e:\n"
"    str     r6, [r4, #0x28]\n"
"    b       loc_fc06488e\n"
"loc_fc064762:\n"
"    bl      sub_fc073caa\n"
"    b       loc_fc06488e\n"
"loc_fc064768:\n"
"    bl      sub_fc073e9a\n"
"    b       loc_fc06477a\n"
"loc_fc06476e:\n"
"    ldr     r0, [r0, #0x10]\n"
"    bl      sub_fc07b766\n"
"    b       loc_fc06488e\n"
"loc_fc064776:\n"
"    bl      sub_fc073efc\n"
"loc_fc06477a:\n"
"    bl      sub_fc079112\n"
"    b       loc_fc06488e\n"
"loc_fc064780:\n"
"    ldr     r0, [r4, #0x54]\n"
"    bl      sub_fc06f596\n"
"    b       loc_fc06488e\n"
"loc_fc064788:\n"
"    bl      sub_fc06f806\n"
"    b       loc_fc06488e\n"
"loc_fc06478e:\n"
"    ldr     r0, [r0, #0xc]\n"
"    bl      sub_fc06f852\n"
"    b       loc_fc06488e\n"
"loc_fc064796:\n"
"    bl      sub_fc06f986\n"
"    b       loc_fc06488e\n"
"loc_fc06479c:\n"
"    bl      sub_fc06fd52\n"
"    b       loc_fc06488e\n"
"loc_fc0647a2:\n"
"    bl      sub_fc06fde8\n"
"    b       loc_fc06488e\n"
"loc_fc0647a8:\n"
"    ldr     r0, [r0, #0xc]\n"
"    bl      sub_fc073ca4\n"
"    b       loc_fc06488e\n"
"loc_fc0647b0:\n"
"    bl      sub_fc071190\n"
"    b       loc_fc06488e\n"
"loc_fc0647b6:\n"
"    b       loc_fc064880\n"
".ltorg\n" 
// firmware had literal pool here
// 0x0003a05c
// 0x00039e5c
// 0x0000ba40
"loc_fc0647c4:\n"
"    bl      sub_fc071338\n"
"    b       loc_fc06488e\n"
"loc_fc0647ca:\n"
"    bl      sub_fc0713b0\n"
"    b       loc_fc06488e\n"
"loc_fc0647d0:\n"
"    bl      sub_fc07144c\n"
"    b       loc_fc06488e\n"
"loc_fc0647d6:\n"
"    bl      sub_fc0714ee\n"
"    b       loc_fc06488e\n"
"loc_fc0647dc:\n"
"    movs    r0, #0\n"
"    b       loc_fc064800\n"
"loc_fc0647e0:\n"
"    bl      sub_fc071808\n"
"    b       loc_fc06488e\n"
"loc_fc0647e6:\n"
"    bl      sub_fc071868\n"
"    b       loc_fc06488e\n"
"loc_fc0647ec:\n"
"    bl      sub_fc07186c\n"
"    b       loc_fc06488e\n"
"loc_fc0647f2:\n"
"    bl      sub_fc07187c\n"
"    b       loc_fc06488e\n"
"loc_fc0647f8:\n"
"    bl      sub_fc0718f4\n"
"    b       loc_fc06488e\n"
"loc_fc0647fe:\n"
"    movs    r0, #1\n"
"loc_fc064800:\n"
"    bl      sub_fc0716e8\n"
"    b       loc_fc06488e\n"
"loc_fc064806:\n"
"    bl      sub_fc074056\n"
"    b       loc_fc06488e\n"
"loc_fc06480c:\n"
"    movs    r0, #0\n"
"    b       loc_fc064812\n"
"loc_fc064810:\n"
"    ldr     r0, [r0, #0xc]\n"
"loc_fc064812:\n"
"    bl      sub_fc0740b6\n"
"    b       loc_fc06488e\n"
"loc_fc064818:\n"
"    bl      sub_fc071614\n"
"    b       loc_fc06488e\n"
"loc_fc06481e:\n"
"    bl      sub_fc07167a\n"
"    b       loc_fc06488e\n"
"loc_fc064824:\n"
"    bl      sub_fc07bf10\n"
"    b       loc_fc06488e\n"
"loc_fc06482a:\n"
"    bl      sub_fc079a30\n"
"    b       loc_fc06488e\n"
"loc_fc064830:\n"
"    bl      sub_fc29d31e\n"
"    b       loc_fc06488e\n"
"loc_fc064836:\n"
"    bl      sub_fc29d3d8\n"
"    b       loc_fc06488e\n"
"loc_fc06483c:\n"
"    ldr     r0, [r0, #0xc]\n"
"    bl      sub_fc0719d0\n"
"    b       loc_fc06488e\n"
"loc_fc064844:\n"
"    bl      sub_fc071a38\n"
"    b       loc_fc06488e\n"
"loc_fc06484a:\n"
"    bl      sub_fc0748be\n"
"    ldrh.w  r0, [r4, #0x1a4]\n"
"    cmp     r0, #4\n"
"    beq     loc_fc064860\n"
"    ldrh    r0, [r4]\n"
"    sub.w   r1, r0, #0x4200\n"
"    subs    r1, #0x36\n"
"    bne     loc_fc06488e\n"
"loc_fc064860:\n"
"    bl      sub_fc29d3d8\n"
"    bl      sub_fc29d8ee\n"
"    bl      sub_fc29d736\n"
"    b       loc_fc06488e\n"
"loc_fc06486e:\n"
"    movs    r2, #0\n"
"    movs    r1, #0x11\n"
"    b       loc_fc064878\n"
"loc_fc064874:\n"
"    movs    r2, #0\n"
"    movs    r1, #0x10\n"
"loc_fc064878:\n"
"    movs    r0, #0\n"
"    bl      sub_fc079cf0\n"
"    b       loc_fc06488e\n"
"loc_fc064880:\n"
"    movw    r2, #0x57e\n"
"    ldr     r1, =0xfc064364\n" //  *"SsShootTask.c"
"    movs    r0, #0\n"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc06488e:\n"
// debug after message handled
#ifdef CAPTSEQ_DEBUG_LOG
"ldr     r0, [sp]\n"
"ldr     r0, [r0]\n"
"bl log_capt_seq2\n"
#endif
"    ldr     r0, [sp]\n"
"    ldr     r1, [r0, #4]\n"
"    ldr     r0, [r5, #4]\n"
"    blx     sub_fc2c7cc8\n" // j_SetEventFlag
"    ldr     r7, [sp]\n"
"    ldr     r0, [r7, #8]\n"
"    cbnz    r0, loc_fc0648ac\n"
"    movw    r2, #0x12b\n"
"    ldr     r1, =0xfc064364\n" //  *"SsShootTask.c"
"    movs    r0, #0\n"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc0648ac:\n"
"    str     r6, [r7, #8]\n"
"    b       loc_fc0646ba\n"
".ltorg\n"
    );
}

//-f=chdk -s=0xfc07b6bb -c=57 
void __attribute__((naked,noinline)) sub_fc07b6ba_my() {
    asm volatile (
"    push    {r3, r4, r5, r6, r7, lr}\n"
"    bl      sub_fc07fa84\n"
"    mov     r4, r0\n"
"    movs    r0, #0xc\n"
"    bl      sub_fc29dbf4\n"
"    ldr     r6, =0x00014684\n"
"    lsls    r0, r0, #0x1f\n"
"    mov.w   r5, #1\n"
"    bne     loc_fc07b758\n"
"    movs    r2, #2\n"
"    mov     r1, sp\n"
"    movw    r0, #0x110\n"
"    bl      _GetPropertyCase\n" //  PROPCASE_TV (272)
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc07b6ee\n"
"    movs    r0, #0\n"
"    movw    r2, #0x17d\n"
"    ldr     r1, =0xfc07b838\n" //  *"SsCaptureCtrl.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc07b6ee:\n"
"    ldrsh.w r0, [sp]\n"
"    bl      sub_fc0b2f18\n"
"    bl      sub_fc0b2d2e\n"
"    cmp     r0, #1\n"
"    bls     loc_fc07b75c\n"
"    movs    r0, #0\n"
"    bl      sub_fc0b2de6\n"
"    bl      sub_fc0a8f60\n"
"    bl      sub_fc073cb0\n"
"    bl      sub_fc082184\n"
"    mov     r1, r4\n"
//"    bl      _captseq_raw_addr_init\n"
"bl captseq_raw_addr_init_my\n"
"    movs    r2, #4\n"
"    movw    r0, #0x116\n"
"    add.w   r1, r4, #0x58\n"
"    bl      _SetPropertyCase\n" //  (278)
"    movs    r2, #4\n"
"    movs    r0, #0x30\n"
"    add.w   r1, r4, #0x5c\n"
"    bl      _SetPropertyCase\n" //  (48)
"    movs    r2, #4\n"
"    movs    r0, #0x44\n"
"    add.w   r1, r4, #8\n"
"    bl      _SetPropertyCase\n" //  (68)
"    bl      sub_fc080b66\n"
"    mvn     r1, #0x1000\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    mov     r0, r4\n"
"    bl      sub_fc07b29a\n"
"    mov     r0, r4\n"
//"    bl      sub_fc083ab0\n"
"    bl      sub_fc083ab0_my\n" // -> remote hook, raw hook
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc07b75a\n"
"loc_fc07b758:\n"
"    str     r5, [r6]\n"
"loc_fc07b75a:\n"
"    pop     {r3, r4, r5, r6, r7, pc}\n"
"loc_fc07b75c:\n"
"    bl      sub_fc079a26\n"
"    bl      sub_fc29dc2c\n"
"    pop     {r3, r4, r5, r6, r7, pc}\n"
".ltorg\n"
    );
}

// -f=chdk -s=0xfc07b4ef -c=173
void __attribute__((naked,noinline)) sub_fc07b4ee_my() {
    asm volatile (
"    push    {r2, r3, r4, r5, r6, lr}\n"
"    ldr     r6, =0x00039e5c\n"
"    mov     r4, r0\n"
"    movs    r5, #0\n"
"    ldr     r0, [r6, #0x28]\n"
"    cbz     r0, loc_fc07b516\n"
"    ldr     r0, =0x00014684\n"
"    ldr     r0, [r0]\n"
"    cbz     r0, loc_fc07b502\n"
"    movs    r5, #0x1d\n"
"loc_fc07b502:\n"
"    mov     r2, r4\n"
"    movs    r1, #2\n"
"    mov     r0, r5\n"
"    bl      sub_fc079cf0\n"
"    mov     r1, r5\n"
"    mov     r0, r4\n"
"    bl      sub_fc080e1a\n"
"    b       loc_fc07b6b4\n"
"loc_fc07b516:\n"
"    bl      sub_fc082184\n"
"    mov     r1, r4\n"
//"    bl      _captseq_raw_addr_init\n"
"bl captseq_raw_addr_init_my\n"
"    movs    r2, #4\n"
"    movw    r0, #0x116\n"
"    add.w   r1, r4, #0x58\n"
"    bl      _SetPropertyCase\n" //  (278)
"    movs    r2, #4\n"
"    movs    r0, #0x30\n"
"    add.w   r1, r4, #0x5c\n"
"    bl      _SetPropertyCase\n" //  (48)
"    ldr.w   r0, [r6, #0x10c]\n"
"    cbnz    r0, loc_fc07b550\n"
"    ldrh.w  r0, [r6, #0x1a2]\n"
"    cmp     r0, #3\n"
"    beq     loc_fc07b556\n"
"    ldr     r0, [r4, #8]\n"
"    cmp     r0, #1\n"
"    bhi     loc_fc07b566\n"
"    b       loc_fc07b556\n"
"loc_fc07b550:\n"
"    ldr     r0, [r4, #0xc]\n"
"    cmp     r0, #1\n"
"    bne     loc_fc07b566\n"
"loc_fc07b556:\n"
"    movs    r0, #0xc\n"
"    bl      sub_fc29dbf4\n"
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc07b566\n"
"    bl      sub_fc079a26\n"
"    movs    r5, #1\n"
"loc_fc07b566:\n"
"    lsls    r0, r5, #0x1f\n"
"    bne     loc_fc07b668\n"
"    ldr.w   r0, [r6, #0xec]\n"
"    cbz     r0, loc_fc07b58a\n"
"    ldrh.w  r0, [r6, #0x1a2]\n"
"    cmp     r0, #3\n"
"    beq     loc_fc07b57e\n"
"    ldr     r0, [r4, #8]\n"
"    cmp     r0, #1\n"
"    bhi     loc_fc07b5ca\n"
"loc_fc07b57e:\n"
"    ldr.w   r0, [r6, #0x10c]\n"
"    cbz     r0, loc_fc07b58a\n"
"    ldr     r0, [r4, #0xc]\n"
"    cmp     r0, #1\n"
"    bhi     loc_fc07b5ca\n"
"loc_fc07b58a:\n"
"    movs    r2, #2\n"
"    movw    r0, #0x110\n"
"    add     r1, sp, #4\n"
"    bl      _GetPropertyCase\n" //  PROPCASE_TV (272)
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc07b5a4\n"
"    movs    r2, #0xcd\n"
"    movs    r0, #0\n"
"    ldr     r1, =0xfc07b838\n" //  *"SsCaptureCtrl.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc07b5a4:\n"
"    ldrsh.w r0, [sp, #4]\n"
"    bl      sub_fc0b2f18\n"
"    bl      sub_fc0b2d2e\n"
"    cmp     r0, #1\n"
"    bls     loc_fc07b5bc\n"
"    movs    r0, #0\n"
"    bl      sub_fc0b2de6\n"
"    b       loc_fc07b5ca\n"
"loc_fc07b5bc:\n"
"    bl      sub_fc079a26\n"
"    bl      sub_fc29dc2c\n"
"    movs    r5, #1\n"
"    lsls    r0, r5, #0x1f\n"
"    bne     loc_fc07b6a2\n"
"loc_fc07b5ca:\n"
"    mov     r0, r4\n"
"    bl      sub_fc07bf5a\n"
"    mov     r5, r0\n"
"    lsls    r0, r0, #0x1f\n"
"    bne     loc_fc07b6a2\n"
"    bl      sub_fc080b66\n"
"    mvn     r1, #0x1000\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    mov     r0, r4\n"
"    bl      sub_fc07d3c6\n"
"    mov     r5, r0\n"
"    lsls    r0, r0, #0x1f\n"
"    bne     loc_fc07b6a2\n"
"    bl      sub_fc0a8f60\n"
"    bl      sub_fc073cb0\n"
"    mov     r0, r4\n"
"    bl      sub_fc07b29a\n"
"    ldr.w   r0, [r6, #0x130]\n"
"    cbnz    r0, loc_fc07b610\n"
"    ldrh.w  r0, [r6, #0x1a2]\n"
"    cmp     r0, #3\n"
"    beq     loc_fc07b610\n"
"    ldr     r0, [r4, #8]\n"
"    cmp     r0, #1\n"
"    bhi     loc_fc07b616\n"
"loc_fc07b610:\n"
"    movs    r0, #2\n"
"    bl      sub_fc074942\n"
"loc_fc07b616:\n"
"    ldr.w   r0, [r6, #0xa8]\n"
"    cmp     r0, #0\n"
"    beq     loc_fc07b68c\n"
"    ldrh.w  r0, [r6, #0x1a2]\n"
"    movw    r5, #0x1000\n"
"    cmp     r0, #3\n"
"    beq     loc_fc07b646\n"
"    ldr     r0, [r4, #8]\n"
"    cmp     r0, #1\n"
"    bls     loc_fc07b646\n"
"    bl      sub_fc080b66\n"
"    movw    r3, #0x119\n"
"    movw    r2, #0x3a98\n"
"    mov     r1, r5\n"
"    str     r3, [sp]\n"
"    ldr     r3, =0xfc07b838\n" //  *"SsCaptureCtrl.c"
"    bl      sub_fc29dda4\n"
"loc_fc07b646:\n"
"    movs    r2, #4\n"
"    movw    r0, #0x188\n"
"    add     r1, sp, #4\n"
"    bl      _GetPropertyCase\n" //  (392)
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc07b662\n"
"    movs    r0, #0\n"
"    movw    r2, #0x11d\n"
"    ldr     r1, =0xfc07b838\n" //  *"SsCaptureCtrl.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc07b662:\n"
"    ldr     r0, [sp, #4]\n"
"    cbnz    r0, loc_fc07b676\n"
"    b       loc_fc07b66a\n"
"loc_fc07b668:\n"
"    b       loc_fc07b6a2\n"
"loc_fc07b66a:\n"
"    bl      sub_fc080b66\n"
"    mov     r1, r5\n"
"    blx     sub_fc2c7cc8\n" // j_SetEventFlag
"    b       loc_fc07b68c\n"
"loc_fc07b676:\n"
"    bl      sub_fc080b66\n"
"    mov     r1, r5\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    ldr     r2, =0xfc07b4dd\n"
"    mov     r3, r5\n"
"    ldr     r0, [sp, #4]\n"
"    mov     r1, r2\n"
"    bl      sub_fc326ef8\n"
"loc_fc07b68c:\n"
"    ldr.w   r0, [r6, #0xb4]\n"
"    cmp     r0, #0\n"
"    mov     r0, r4\n"
"    beq     loc_fc07b69c\n"
"    bl      sub_fc083f42\n"
"    b       loc_fc07b6b4\n"
"loc_fc07b69c:\n"
"    bl      sub_fc083ab0_my\n" // -> remote, raw hook etc
"    b       loc_fc07b6b4\n"
"loc_fc07b6a2:\n"
"    movs    r1, #2\n"
"    mov     r2, r4\n"
"    mov     r0, r5\n"
"    bl      sub_fc079cf0\n"
"    mov     r1, r5\n"
"    mov     r0, r4\n"
"    bl      sub_fc080f50\n"
"loc_fc07b6b4:\n"
"    movs    r0, #0\n"
"    str     r0, [r6, #0x28]\n"
"    pop     {r2, r3, r4, r5, r6, pc}\n"
".ltorg\n"
    );
}

#ifdef CAPTSEQ_DEBUG_LOG
void log_nr_call(void) {
    _LogCameraEvent(0x60,"nr hook %d",_nrflag);
}
void log_remote_hook(void) {
    _LogCameraEvent(0x60,"remote hook");
}
void log_rh(void) {
    _LogCameraEvent(0x60,"raw hook rb:0x%08x rbc:0x%08x",hook_raw_image_addr(),current_raw_addr);
}
#endif

// -f=chdk -s=0xfc083ab1 -c=210
void __attribute__((naked,noinline)) sub_fc083ab0_my() {
    asm volatile (
"    push.w  {r1, r2, r3, r4, r5, r6, r7, r8, sb, lr}\n"
"    ldr     r7, =0x00039e5c\n"
"    mov     r4, r0\n"
"    ldr.w   r0, [r7, #0x16c]\n"
"    cbz     r0, loc_fc083acc\n"
"    ldrh.w  r0, [r7, #0x1a2]\n"
"    cmp     r0, #3\n"
"    beq     loc_fc083acc\n"
"    ldr     r0, [r4, #8]\n"
"    cmp     r0, #1\n"
"    bhi     loc_fc083ad6\n"
"loc_fc083acc:\n"
"    mov     r0, r4\n"
"    bl      sub_fc07b230\n"
"    bl      sub_fc07bc88\n"
"loc_fc083ad6:\n"
"    ldr.w   r0, [r7, #0xa4]\n"
"    cbnz    r0, loc_fc083af2\n"
"    ldrh.w  r0, [r7, #0x1a2]\n"
"    cmp     r0, #3\n"
"    beq     loc_fc083aea\n"
"    ldr     r0, [r4, #8]\n"
"    cmp     r0, #1\n"
"    bhi     loc_fc083af2\n"
"loc_fc083aea:\n"
"    bl      sub_fc0740a6\n"
"    bl      sub_fc07b86a\n"
"loc_fc083af2:\n"
"    movs    r2, #4\n"
"    movw    r0, #0x137\n"
"    add     r1, sp, #8\n"
"    bl      _GetPropertyCase\n" //  (311)
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc083b0e\n"
"    movs    r0, #0\n"
"    movw    r2, #0x150\n"
"    ldr     r1, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc083b0e:\n"
"    ldr     r0, [sp, #8]\n"
"    ubfx    r0, r0, #8, #8\n"
"    cmp     r0, #6\n"
"    bne     loc_fc083b1e\n"
"    ldr     r0, =0xfc083a79\n"
"    movs    r1, #0\n"
"    b       loc_fc083b22\n"
"loc_fc083b1e:\n"
"    ldr     r0, =0xfc07ae63\n"
"    mov     r1, r4\n"
"loc_fc083b22:\n"
"    bl      sub_fc0b8fc8\n"
"    movs    r2, #2\n"
"    movw    r0, #0x112\n"
"    add     r1, sp, #4\n"
"    bl      _GetPropertyCase\n" //  PROPCASE_USER_TV (274)
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc083b42\n"
"    movs    r0, #0\n"
"    movw    r2, #0x159\n"
"    ldr     r1, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc083b42:\n"
"    ldr.w   r0, [r7, #0x18c]\n"
"    cbz     r0, loc_fc083b4c\n"
"    bl      sub_fc07b168\n"
"loc_fc083b4c:\n"
"    movs    r0, #0\n"
"    bl      sub_fc0743b6\n"
"bl capt_seq_hook_set_nr\n"
#ifdef CAPTSEQ_DEBUG_LOG
"bl log_nr_call\n"
#endif
"    mov     r0, r4\n"
"    bl      sub_fc07b360\n"
"    ldr     r1, =0x000219e4\n"
"    movs    r2, #4\n"
"    movs    r0, #0x90\n"
"    bl      _GetPropertyCase\n" //  (144)
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc083b72\n"
"    movs    r0, #0\n"
"    movw    r2, #0x16e\n"
"    ldr     r1, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc083b72:\n"
"    bl      sub_fc074470\n"
"    bl      sub_fc07b3f2\n"
"    movs    r1, #0\n"
"    mov     r0, r4\n"
"    bl      sub_fc083a7a\n"
"    mov     r6, r0\n"
"    BL      wait_until_remote_button_is_released\n" // + remote hook, might be able to go later
#ifdef CAPTSEQ_DEBUG_LOG
"bl log_remote_hook\n"
#endif
"    ldr     r0, [sp, #8]\n"
"    ubfx    r0, r0, #8, #8\n"
"    cmp     r0, #6\n"
"    bne     loc_fc083b92\n"
"    ldr     r2, =0xfc07b137\n"
"    b       loc_fc083b94\n"
"loc_fc083b92:\n"
"    ldr     r2, =0xfc07b14d\n"
"loc_fc083b94:\n"
"    ldrh    r0, [r4, #0x18]\n"
"    ldr     r5, =0x000219e4\n"
"    cbz     r0, loc_fc083ba2\n"
"    cmp     r0, #1\n"
"    beq     loc_fc083bb8\n"
"    cmp     r0, #4\n"
"    bne     loc_fc083c1e\n"
"loc_fc083ba2:\n"
"    str     r6, [sp]\n"
"    mov     r3, r2\n"
"    ldr     r1, [r4, #0x5c]\n"
"    mov     r0, r4\n"
"    ldr     r2, [r5]\n"
"    bl      sub_fc07ac8e\n"
"    mov     r5, r0\n"
"    bl      sub_fc13400e\n"
"    b       loc_fc083c2c\n"
"loc_fc083bb8:\n"
"    str     r6, [sp]\n"
"    mov     r3, r2\n"
"    ldr     r1, [r4, #0x5c]\n"
"    mov     r8, r5\n"
"    ldr     r2, [r5]\n"
"    mov     r0, r4\n"
"    bl      sub_fc07acf8\n"
"    movs    r2, #1\n"
"    mov     r5, r0\n"
"    movs    r1, #0\n"
"    movs    r0, #0x45\n"
"    bl      sub_fc268ca4\n"
"    lsls    r0, r5, #0x1f\n"
"    bne     loc_fc083c2c\n"
"    ldr.w   r0, [r7, #0x100]\n"
"    cbz     r0, loc_fc083bee\n"
"    ldr     r1, [r4, #8]\n"
"    ldr     r2, =0x001d7f7c\n"
"    ldr     r0, [r4, #0x5c]\n"
"    add.w   r1, r2, r1, lsl #2\n"
"    str     r0, [r1, #-0x4]\n"
"    b       loc_fc083c18\n"
"loc_fc083bee:\n"
"    ldr     r0, =0xfc083a79\n"
"    movs    r1, #0\n"
"    bl      sub_fc0b8fc8\n"
"    movs    r1, #1\n"
"    mov     r0, r4\n"
"    bl      sub_fc083a7a\n"
"    mov     r6, r0\n"
"    ldr.w   r0, [r8]\n"
"    mov     r5, r8\n"
"    bl      sub_fc07b1d2\n"
"    ldr     r1, [r4, #0x5c]\n"
"    mov     r3, r6\n"
"    ldr     r2, [r5]\n"
"    mov     r0, r4\n"
"    bl      sub_fc07ad70\n"
"    mov     r5, r0\n"
"loc_fc083c18:\n"
"    bl      sub_fc07ac38\n"
"    b       loc_fc083c2c\n"
"loc_fc083c1e:\n"
"    movs    r0, #0\n"
"    movw    r2, #0x1a7\n"
"    ldr     r1, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"    movs    r5, #0x1d\n"
"loc_fc083c2c:\n"
"    bl      sub_fc07b3f6\n"
"    ldr.w   r8, =0xfc083a79\n"
"    lsls    r0, r5, #0x1f\n"
"    bne     loc_fc083c88\n"
"    ldr.w   r0, [r7, #0x108]\n"
"    cbnz    r0, loc_fc083c54\n"
"    mov     r0, r4\n"
"    bl      sub_fc08107a\n"
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc083c54\n"
"    movs    r0, #0\n"
"    movw    r2, #0x1cd\n"
"    ldr     r1, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc083c54:\n"
#ifdef CAPTSEQ_DEBUG_LOG
"bl log_rh\n"
#endif
"    BL      capt_seq_hook_raw_here\n"
"    BL      clear_current_raw_addr\n"
"    mov     r0, r4\n"
"    bl      sub_fc07b3e2\n"
"    mov     r0, r4\n"
"    bl      sub_fc07b3b6\n"
"    cmp     r6, r8\n"
"    beq     loc_fc083c88\n"
"    bl      sub_fc080b66\n"
"    movs    r1, #4\n"
"    movw    sb, #0x1da\n"
"    ldr     r3, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    movw    r2, #0x3a98\n"
"    str.w   sb, [sp]\n"
"    bl      sub_fc29dda4\n"
"    cbz     r0, loc_fc083c88\n"
"    movs    r0, #0\n"
"    mov     r2, sb\n"
"    ldr     r1, =0xfc083e94\n" //  *"SsStandardCaptureSeq.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc083c88:\n"
"    ldr.w   r0, [r7, #0x18c]\n"
"    cbz     r0, loc_fc083c98\n"
"    movs    r2, #1\n"
"    movs    r1, #0\n"
"    movs    r0, #0x46\n"
"    bl      sub_fc268ca4\n"
"loc_fc083c98:\n"
"    movs    r1, #2\n"
"    mov     r2, r4\n"
"    mov     r0, r5\n"
"    bl      sub_fc079cf0\n"
"    ldr     r0, [r7, #0x28]\n"
"    cmp     r0, #0\n"
"    mov     r0, r8\n"
"    beq     loc_fc083cbe\n"
"    cmp     r6, r0\n"
"    beq     loc_fc083cb2\n"
"    movs    r1, #1\n"
"    b       loc_fc083cb4\n"
"loc_fc083cb2:\n"
"    movs    r1, #0\n"
"loc_fc083cb4:\n"
"    mov     r2, r5\n"
"    mov     r0, r4\n"
"    bl      sub_fc080dd8\n"
"    b       loc_fc083cd0\n"
"loc_fc083cbe:\n"
"    cmp     r6, r0\n"
"    beq     loc_fc083cc6\n"
"    movs    r1, #1\n"
"    b       loc_fc083cc8\n"
"loc_fc083cc6:\n"
"    movs    r1, #0\n"
"loc_fc083cc8:\n"
"    mov     r2, r5\n"
"    mov     r0, r4\n"
"    bl      sub_fc080d92\n"
"loc_fc083cd0:\n"
"    mov     r0, r5\n"
"    pop.w   {r1, r2, r3, r4, r5, r6, r7, r8, sb, pc}\n"
".ltorg\n"
    );
}

// exp_drv not needed for extended exposure, probably works up to 1024s but required for shorter than 1/3200
// -f=chdk -s=task_ExpDrv -c=428
// note, breaks on literal pool around fc2867ea-fc286814
// task_ExpDrv 0xfc28653b
void __attribute__((naked,noinline)) exp_drv_task() {
    asm volatile (
"    push.w  {r4, r5, r6, r7, r8, sb, sl, fp, lr}\n"
"    sub     sp, #0x2c\n"
"    ldr.w   sb, =0x0000d7a4\n"
"    ldr.w   sl, =0xfffff400\n"
"    movs    r0, #0\n"
"    ldr     r6, =0x0005c174\n"
"    add.w   r8, sp, #0x1c\n"
"    movw    fp, #0xbb8\n"
"    str     r0, [sp, #0xc]\n"
"loc_fc286556:\n"
"    ldr.w   r0, [sb, #0x20]\n"
"    movs    r2, #0\n"
"    add     r1, sp, #0x28\n"
"    mov     r4, sb\n"
"    blx     sub_fc2c7ca0\n" // j_ReceiveMessageQueue
"    ldr     r0, [sp, #0xc]\n"
"    cmp     r0, #1\n"
"    bne     loc_fc28658e\n"
"    ldr     r0, [sp, #0x28]\n"
"    ldr     r0, [r0]\n"
"    cmp     r0, #0x15\n"
"loc_fc286570:\n"
"    beq     loc_fc286670\n"
"    cmp     r0, #0x16\n"
"    beq     loc_fc286570\n"
"    cmp     r0, #0x17\n"
"    beq     loc_fc286570\n"
"    cmp     r0, #0x18\n"
"    beq     loc_fc286570\n"
"    cmp     r0, #0x19\n"
"    beq     loc_fc286680\n"
"    cmp     r0, #0x30\n"
"    beq     loc_fc28662a\n"
"    movs    r0, #0\n"
"    add     r1, sp, #0xc\n"
"    bl      sub_fc2864f6\n"
"loc_fc28658e:\n"
"    ldr     r0, [sp, #0x28]\n"
"    ldr     r1, [r0]\n"
"    cmp     r1, #0x38\n"
"    bne     loc_fc2865ac\n"
"    bl      sub_fc287448\n"
"    ldr.w   r0, [sb, #0x1c]\n"
"    movs    r1, #1\n"
"    blx     sub_fc2c7cc8\n" // j_SetEventFlag
"    blx     sub_fc2c7cf8\n" // -> ExitTask
"    add     sp, #0x2c\n"
//"    b       loc_fc286280\n"
"    pop.w   {r4, r5, r6, r7, r8, sb, sl, fp, pc}\n" // + @fc286280
"loc_fc2865ac:\n"
"    cmp     r1, #0x37\n"
"    bne     loc_fc2865be\n"
"    add.w   r0, r0, #0xac\n"
"    ldrd    r2, r1, [r0]\n"
"    mov     r0, r1\n"
"    blx     r2\n"
"    b       loc_fc2869d2\n"
"loc_fc2865be:\n"
"    cmp     r1, #0x2e\n"
"    bne     loc_fc2865f4\n"
"    ldr     r0, [r4, #0x1c]\n"
"    movs    r1, #0x80\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    ldr     r0, =0xfc28235d\n"
"    movs    r1, #0x80\n"
"    bl      sub_fc0d727a\n"
"    ldr     r0, [r4, #0x1c]\n"
"    movs    r1, #0x80\n"
"    mov     r2, fp\n"
"    blx     sub_fc2c7cd0\n" // j_WaitForAllEventFlag
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc2865e6\n"
"    movw    r2, #0x199e\n"
"    b       loc_fc286676\n"
"loc_fc2865e6:\n"
"    ldr     r1, [sp, #0x28]\n"
"    add.w   r1, r1, #0xac\n"
"    ldrd    r1, r0, [r1]\n"
"    blx     r1\n"
"    b       loc_fc2869d2\n"
"loc_fc2865f4:\n"
"    cmp     r1, #0x2f\n"
"    bne     loc_fc286626\n"
"    add     r1, sp, #0xc\n"
"    bl      sub_fc2864f6\n"
"    movw    r5, #0x100\n"
"    ldr     r0, [r4, #0x1c]\n"
"    mov     r1, r5\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    ldr     r0, =0xfc282367\n"
"    mov     r1, r5\n"
"    bl      sub_fc0d7c18\n"
"    ldr     r0, [r4, #0x1c]\n"
"    mov     r2, fp\n"
"    mov     r1, r5\n"
"    blx     sub_fc2c7cd0\n" // j_WaitForAllEventFlag
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc2865e6\n"
"    movw    r2, #0x19a8\n"
"    b       loc_fc286676\n"
"loc_fc286626:\n"
"    cmp     r1, #0x30\n"
"    bne     loc_fc286634\n"
"loc_fc28662a:\n"
"    ldr     r0, [sp, #0x28]\n"
"    add     r1, sp, #0xc\n"
"    bl      sub_fc2864f6\n"
"    b       loc_fc2865e6\n"
"loc_fc286634:\n"
"    cmp     r1, #0x35\n"
"    bne     loc_fc286646\n"
"    bl      sub_fc3a0e74\n"
"    bl      sub_fc130f3c\n"
"    bl      sub_fc130c1e\n"
"    b       loc_fc2865e6\n"
"loc_fc286646:\n"
"    cmp     r1, #0x36\n"
"    bne     loc_fc286680\n"
"    ldr     r0, [r4, #0x1c]\n"
"    movs    r1, #4\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    ldr     r1, =0xfc28237b\n"
"    movs    r2, #4\n"
"    mov     r0, sl\n"
"    bl      sub_fc28749a\n"
"    bl      sub_fc3a0bd6\n"
"    ldr     r0, [r4, #0x1c]\n"
"    movs    r1, #4\n"
"    mov     r2, fp\n"
"    blx     sub_fc2c7dd0\n" // j_WaitForAnyEventFlag
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc2865e6\n"
"    b       loc_fc286672\n"
"loc_fc286670:\n"
"    b       loc_fc286680\n"
"loc_fc286672:\n"
"    movw    r2, #0x1a84\n"
"loc_fc286676:\n"
"    ldr     r1, =0xfc282b80\n" //  **"ExpDrv.c"
"    movs    r0, #0\n"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"    b       loc_fc2865e6\n"
"loc_fc286680:\n"
"    ldr     r0, [sp, #0x28]\n"
"    movs    r5, #1\n"
"    ldr     r1, [r0]\n"
"    cmp     r1, #0x13\n"
"    beq     loc_fc28668e\n"
"    cmp     r1, #0x14\n"
"    bne     loc_fc2866cc\n"
"loc_fc28668e:\n"
"    ldr.w   r1, [r0, #0x94]\n"
"    mov     r4, r8\n"
"    add.w   r1, r1, r1, lsl #1\n"
"    add.w   r1, r0, r1, lsl #2\n"
"    subs    r1, #8\n"
"    ldm     r1!, {r2, r3, r7}\n"
"    stm     r4!, {r2, r3, r7}\n"
"    bl      sub_fc284dfa\n"
"    ldr     r0, [sp, #0x28]\n"
"    add.w   r0, r0, #0x94\n"
"    ldrd    r3, r2, [r0, #0x18]\n"
"    ldr     r1, [r0]\n"
"    sub.w   r0, r0, #0x90\n"
"    blx     r3\n"
"    ldr     r0, [sp, #0x28]\n"
"    bl      sub_fc2876b2\n"
"    ldr     r0, [sp, #0x28]\n"
"    add.w   r0, r0, #0x94\n"
"    ldr     r1, [r0]\n"
"    ldrd    r3, r2, [r0, #0x20]\n"
"    b       loc_fc2868f8\n"
"loc_fc2866cc:\n"
"    cmp     r1, #0x15\n"
"    beq     loc_fc2866e0\n"
"    cmp     r1, #0x16\n"
"    beq     loc_fc2866e0\n"
"    cmp     r1, #0x17\n"
"    beq     loc_fc2866e0\n"
"    cmp     r1, #0x18\n"
"    beq     loc_fc2866e0\n"
"    cmp     r1, #0x19\n"
"    bne     loc_fc286752\n"
"loc_fc2866e0:\n"
"    add     r3, sp, #0xc\n"
"    mov     r2, sp\n"
"    add     r1, sp, #0x1c\n"
"    bl      sub_fc284fca\n"
"    cmp     r0, #1\n"
"    mov     r4, r0\n"
"    beq     loc_fc2866f4\n"
"    cmp     r4, #5\n"
"    bne     loc_fc28670a\n"
"loc_fc2866f4:\n"
"    ldr     r0, [sp, #0x28]\n"
"    mov     r2, r4\n"
"    add.w   r0, r0, #0x94\n"
"    ldrd    r7, r3, [r0, #0x18]\n"
"    ldr     r1, [r0]\n"
"    sub.w   r0, r0, #0x90\n"
"    blx     r7\n"
"    b       loc_fc286730\n"
"loc_fc28670a:\n"
"    cmp     r4, #2\n"
"    beq     loc_fc286712\n"
"    cmp     r4, #6\n"
"    bne     loc_fc28673c\n"
"loc_fc286712:\n"
"    ldr     r0, [sp, #0x28]\n"
"    mov     r2, r4\n"
"    add.w   r0, r0, #0x94\n"
"    ldrd    r7, r3, [r0, #0x18]\n"
"    ldr     r1, [r0]\n"
"    sub.w   r0, r0, #0x90\n"
"    blx     r7\n"
"    ldr     r0, [sp, #0x28]\n"
"    add     r1, sp, #0x1c\n"
"    mov     r2, sp\n"
"    bl      sub_fc286284\n"
"loc_fc286730:\n"
"    ldr     r2, [sp, #0xc]\n"
"    mov     r1, r4\n"
"    ldr     r0, [sp, #0x28]\n"
"    bl      sub_fc2864a6\n"
"    b       loc_fc2868fe\n"
"loc_fc28673c:\n"
"    ldr     r0, [sp, #0x28]\n"
"    mov     r2, r4\n"
"    add.w   r0, r0, #0x94\n"
"    ldrd    r7, r3, [r0, #0x18]\n"
"    ldr     r1, [r0]\n"
"    sub.w   r0, r0, #0x90\n"
"    blx     r7\n"
"    b       loc_fc2868fe\n"
"loc_fc286752:\n"
"    cmp     r1, #0x2a\n"
"    beq     loc_fc28675a\n"
"    cmp     r1, #0x2b\n"
"    bne     loc_fc28678c\n"
"loc_fc28675a:\n"
"    ldr.w   r1, [r0, #0x94]\n"
"    mov     r4, r8\n"
"    add.w   r1, r1, r1, lsl #1\n"
"    add.w   r1, r0, r1, lsl #2\n"
"    subs    r1, #8\n"
"    ldm     r1!, {r2, r3, r7}\n"
"    stm     r4!, {r2, r3, r7}\n"
"    bl      sub_fc283f2e\n"
"    ldr     r0, [sp, #0x28]\n"
"    add.w   r0, r0, #0x94\n"
"    ldrd    r3, r2, [r0, #0x18]\n"
"    ldr     r1, [r0]\n"
"    sub.w   r0, r0, #0x90\n"
"    blx     r3\n"
"    ldr     r0, [sp, #0x28]\n"
"    bl      sub_fc284318\n"
"    b       loc_fc2868fe\n"
"loc_fc28678c:\n"
"    adds    r1, r0, #4\n"
"    mov     r4, r8\n"
"    ldm     r1!, {r2, r3, r7}\n"
"    stm     r4!, {r2, r3, r7}\n"
"    ldr     r1, [r0]\n"
"    cmp     r1, #0x2e\n"
"    bhs     loc_fc2867d0\n"
"    tbb     [pc, r1]\n" // (jumptable r1 46 elements)
"branchtable_fc28679e:\n"
"    .byte((loc_fc2867cc - branchtable_fc28679e) / 2)\n" // (case 0)
"    .byte((loc_fc2867cc - branchtable_fc28679e) / 2)\n" // (case 1)
"    .byte((loc_fc2867d2 - branchtable_fc28679e) / 2)\n" // (case 2)
"    .byte((loc_fc2867d8 - branchtable_fc28679e) / 2)\n" // (case 3)
"    .byte((loc_fc2867d8 - branchtable_fc28679e) / 2)\n" // (case 4)
"    .byte((loc_fc2867d8 - branchtable_fc28679e) / 2)\n" // (case 5)
"    .byte((loc_fc2867cc - branchtable_fc28679e) / 2)\n" // (case 6)
"    .byte((loc_fc2867d2 - branchtable_fc28679e) / 2)\n" // (case 7)
"    .byte((loc_fc2867d8 - branchtable_fc28679e) / 2)\n" // (case 8)
"    .byte((loc_fc2867d8 - branchtable_fc28679e) / 2)\n" // (case 9)
"    .byte((loc_fc286814 - branchtable_fc28679e) / 2)\n" // (case 10)
"    .byte((loc_fc286814 - branchtable_fc28679e) / 2)\n" // (case 11)
"    .byte((loc_fc286814 - branchtable_fc28679e) / 2)\n" // (case 12)
"    .byte((loc_fc2868dc - branchtable_fc28679e) / 2)\n" // (case 13)
"    .byte((loc_fc2868e2 - branchtable_fc28679e) / 2)\n" // (case 14)
"    .byte((loc_fc2868e2 - branchtable_fc28679e) / 2)\n" // (case 15)
"    .byte((loc_fc2868e2 - branchtable_fc28679e) / 2)\n" // (case 16)
"    .byte((loc_fc2868e2 - branchtable_fc28679e) / 2)\n" // (case 17)
"    .byte((loc_fc2868e8 - branchtable_fc28679e) / 2)\n" // (case 18)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 19)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 20)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 21)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 22)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 23)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 24)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 25)
"    .byte((loc_fc2867de - branchtable_fc28679e) / 2)\n" // (case 26)
"    .byte((loc_fc2867e4 - branchtable_fc28679e) / 2)\n" // (case 27)
"    .byte((loc_fc2867e4 - branchtable_fc28679e) / 2)\n" // (case 28)
"    .byte((loc_fc2867e4 - branchtable_fc28679e) / 2)\n" // (case 29)
"    .byte((loc_fc28681c - branchtable_fc28679e) / 2)\n" // (case 30)
"    .byte((loc_fc28681c - branchtable_fc28679e) / 2)\n" // (case 31)
"    .byte((loc_fc28681c - branchtable_fc28679e) / 2)\n" // (case 32)
"    .byte((loc_fc28681c - branchtable_fc28679e) / 2)\n" // (case 33)
"    .byte((loc_fc286822 - branchtable_fc28679e) / 2)\n" // (case 34)
"    .byte((loc_fc28684c - branchtable_fc28679e) / 2)\n" // (case 35)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 36)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 37)
"    .byte((loc_fc286876 - branchtable_fc28679e) / 2)\n" // (case 38)
"    .byte((loc_fc2868a0 - branchtable_fc28679e) / 2)\n" // (case 39)
"    .byte((loc_fc2868ca - branchtable_fc28679e) / 2)\n" // (case 40)
"    .byte((loc_fc2868ca - branchtable_fc28679e) / 2)\n" // (case 41)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 42)
"    .byte((loc_fc2868ec - branchtable_fc28679e) / 2)\n" // (case 43)
"    .byte((loc_fc2868d0 - branchtable_fc28679e) / 2)\n" // (case 44)
"    .byte((loc_fc2868d6 - branchtable_fc28679e) / 2)\n" // (case 45)
".align 1\n"
"loc_fc2867cc:\n"
"    bl      sub_fc28291c\n"
"loc_fc2867d0:\n"
"    b       loc_fc2868ec\n"
"loc_fc2867d2:\n"
"    bl      sub_fc282ba6\n"
"    b       loc_fc2868ec\n"
"loc_fc2867d8:\n"
"    bl      sub_fc282da0\n"
"    b       loc_fc2868ec\n"
"loc_fc2867de:\n"
"    bl      sub_fc283040\n"
"    b       loc_fc2868ec\n"
"loc_fc2867e4:\n"
"    bl      sub_fc2831f0\n"
"    b       loc_fc2868ec\n"
// literal pool in original fw
".ltorg\n"
"loc_fc286814:\n" // case 10-12 reconstructed with -f=chdk -s=0xfc286815 -c=3
"    bl      sub_fc28361e_my\n"
"    movs    r5, #0\n"
"    b       loc_fc2868ec\n"
"loc_fc28681c:\n"
"    bl      sub_fc283718\n"
"    b       loc_fc2868ec\n"
"loc_fc286822:\n"
"    ldrh    r1, [r0, #4]\n"
"    strh.w  r1, [sp, #0x1c]\n"
"    ldrh    r1, [r6, #2]\n"
"    strh.w  r1, [sp, #0x1e]\n"
"    ldrh    r1, [r6, #4]\n"
"    strh.w  r1, [sp, #0x20]\n"
"    ldrh    r1, [r6, #6]\n"
"    strh.w  r1, [sp, #0x22]\n"
"    ldrh    r1, [r0, #0xc]\n"
"    strh.w  r1, [sp, #0x24]\n"
"    ldrh    r1, [r6, #0xa]\n"
"    strh.w  r1, [sp, #0x26]\n"
"    bl      sub_fc283c7a\n"
"    b       loc_fc2868ec\n"
"loc_fc28684c:\n"
"    ldrh    r1, [r0, #4]\n"
"    strh.w  r1, [sp, #0x1c]\n"
"    ldrh    r1, [r6, #2]\n"
"    strh.w  r1, [sp, #0x1e]\n"
"    ldrh    r1, [r6, #4]\n"
"    strh.w  r1, [sp, #0x20]\n"
"    ldrh    r1, [r6, #6]\n"
"    strh.w  r1, [sp, #0x22]\n"
"    ldrh    r1, [r6, #8]\n"
"    strh.w  r1, [sp, #0x24]\n"
"    ldrh    r1, [r6, #0xa]\n"
"    strh.w  r1, [sp, #0x26]\n"
"    bl      sub_fc287506\n"
"    b       loc_fc2868ec\n"
"loc_fc286876:\n"
"    ldrh    r1, [r6]\n"
"    strh.w  r1, [sp, #0x1c]\n"
"    ldrh    r1, [r0, #6]\n"
"    strh.w  r1, [sp, #0x1e]\n"
"    ldrh    r1, [r6, #4]\n"
"    strh.w  r1, [sp, #0x20]\n"
"    ldrh    r1, [r6, #6]\n"
"    strh.w  r1, [sp, #0x22]\n"
"    ldrh    r1, [r6, #8]\n"
"    strh.w  r1, [sp, #0x24]\n"
"    ldrh    r1, [r6, #0xa]\n"
"    strh.w  r1, [sp, #0x26]\n"
"    bl      sub_fc28758c\n"
"    b       loc_fc2868ec\n"
"loc_fc2868a0:\n"
"    ldrh    r1, [r6]\n"
"    strh.w  r1, [sp, #0x1c]\n"
"    ldrh    r1, [r6, #2]\n"
"    strh.w  r1, [sp, #0x1e]\n"
"    ldrh    r1, [r6, #4]\n"
"    strh.w  r1, [sp, #0x20]\n"
"    ldrh    r1, [r6, #6]\n"
"    strh.w  r1, [sp, #0x22]\n"
"    ldrh    r1, [r0, #0xc]\n"
"    strh.w  r1, [sp, #0x24]\n"
"    ldrh    r1, [r6, #0xa]\n"
"    strh.w  r1, [sp, #0x26]\n"
"    bl      sub_fc287644\n"
"    b       loc_fc2868ec\n"
"loc_fc2868ca:\n"
"    bl      sub_fc283d58\n"
"    b       loc_fc2868ec\n"
"loc_fc2868d0:\n"
"    bl      sub_fc2843e0\n"
"    b       loc_fc2868ec\n"
"loc_fc2868d6:\n"
"    bl      sub_fc2847c0\n"
"    b       loc_fc2868ec\n"
"loc_fc2868dc:\n"
"    bl      sub_fc2849d4\n"
"    b       loc_fc2868ec\n"
"loc_fc2868e2:\n"
"    bl      sub_fc284b26\n"
"    b       loc_fc2868ec\n"
"loc_fc2868e8:\n"
"    bl      sub_fc284c42\n"
"loc_fc2868ec:\n"
"    ldr     r0, [sp, #0x28]\n"
"    add.w   r0, r0, #0x94\n"
"    ldrd    r3, r2, [r0, #0x18]\n"
"    ldr     r1, [r0]\n"
"loc_fc2868f8:\n"
"    sub.w   r0, r0, #0x90\n"
"    blx     r3\n"
"loc_fc2868fe:\n"
"    ldr     r0, [sp, #0x28]\n"
"    ldr     r0, [r0]\n"
"    cmp     r0, #0x11\n"
"    beq     loc_fc286922\n"
"    bgt     loc_fc286916\n"
"    cmp     r0, #1\n"
"    beq     loc_fc286922\n"
"    cmp     r0, #4\n"
"    beq     loc_fc286922\n"
"    cmp     r0, #0xf\n"
"    bne     loc_fc286950\n"
"    b       loc_fc286922\n"
"loc_fc286916:\n"
"    cmp     r0, #0x14\n"
"    beq     loc_fc286922\n"
"    cmp     r0, #0x19\n"
"    beq     loc_fc286922\n"
"    cmp     r0, #0x1c\n"
"    bne     loc_fc286950\n"
"loc_fc286922:\n"
"    ldrsh.w r1, [r6]\n"
"    mov     r2, sl\n"
"    cmp     r1, sl\n"
"    beq     loc_fc286934\n"
"    ldrsh.w r0, [r6, #8]\n"
"    cmp     r0, r2\n"
"    bne     loc_fc286948\n"
"loc_fc286934:\n"
"    add     r0, sp, #0x10\n"
"    bl      sub_fc3160ea\n"
"    ldrh.w  r0, [sp, #0x10]\n"
"    strh.w  r0, [sp, #0x1c]\n"
"    ldrh.w  r0, [sp, #0x18]\n"
"    b       loc_fc28694c\n"
"loc_fc286948:\n"
"    strh.w  r1, [sp, #0x1c]\n"
"loc_fc28694c:\n"
"    strh.w  r0, [sp, #0x24]\n"
"loc_fc286950:\n"
"    cmp     r5, #1\n"
"    ldr     r0, [sp, #0x28]\n"
"    bne     loc_fc286988\n"
"    movs    r2, #0xc\n"
"    ldr.w   r1, [r0, #0x94]\n"
"    add.w   r1, r1, r1, lsl #1\n"
"    add.w   r4, r0, r1, lsl #2\n"
"    ldr     r0, =0x0005c174\n"
"    subs    r4, #8\n"
"    add     r1, sp, #0x1c\n"
"    blx     sub_fc2c7d80\n"
"    ldr     r0, =0x0005c174\n"
"    movs    r2, #0xc\n"
"    add     r1, sp, #0x1c\n"
"    adds    r0, #0xc\n"
"    blx     sub_fc2c7d80\n"
"    ldr     r0, =0x0005c174\n"
"    movs    r2, #0xc\n"
"    mov     r1, r4\n"
"    adds    r0, #0x18\n"
"    blx     sub_fc2c7d80\n"
"    b       loc_fc2869d2\n"
"loc_fc286988:\n"
"    ldr     r0, [r0]\n"
"    mov.w   r3, #1\n"
"    cmp     r0, #0xc\n"
"    bne     loc_fc2869b2\n"
"    movs    r2, #0\n"
"    mov     r1, r3\n"
"    strd    r2, r3, [sp]\n"
"    movs    r0, #0\n"
"    mov     r2, r3\n"
"    bl      sub_fc282774\n"
"    movs    r3, #1\n"
"    movs    r2, #0\n"
"    mov     r1, r3\n"
"    movs    r0, #0\n"
"    strd    r2, r3, [sp]\n"
"    mov     r2, r3\n"
"    b       loc_fc2869ce\n"
"loc_fc2869b2:\n"
"    movs    r2, #1\n"
"    strd    r2, r3, [sp]\n"
"    mov     r3, r2\n"
"    mov     r1, r2\n"
"    mov     r0, r2\n"
"    bl      sub_fc282774\n"
"    movs    r3, #1\n"
"    str     r3, [sp]\n"
"    mov     r2, r3\n"
"    mov     r1, r3\n"
"    mov     r0, r3\n"
"    str     r3, [sp, #4]\n"
"loc_fc2869ce:\n"
"    bl      sub_fc28289a\n"
"loc_fc2869d2:\n"
"    ldr     r0, [sp, #0x28]\n"
"    bl      sub_fc287448\n"
"    b       loc_fc286556\n"
".ltorg\n"
    );
}

// -f=chdk -s=0xfc28361f -c=87
void __attribute__((naked,noinline)) sub_fc28361e_my() {
    asm volatile (
"    push.w  {r4, r5, r6, r7, r8, lr}\n"
"    ldr     r7, =0x0000d7a4\n"
"    movs    r1, #0x3e\n"
"    mov     r4, r0\n"
"    ldr     r0, [r7, #0x1c]\n"
"    blx     sub_fc2c7dc8\n" // j_ClearEventFlag
"    movs    r2, #0\n"
"    ldrsh.w r0, [r4, #4]\n"
"    movs    r3, #1\n"
"    mov     r1, r2\n"
"    bl      sub_fc2823bc\n"
"    mov     r6, r0\n"
"    ldrsh.w r0, [r4, #6]\n"
"    bl      sub_fc28255a\n"
"    ldrsh.w r0, [r4, #8]\n"
"    bl      sub_fc28259e\n"
"    ldrsh.w r0, [r4, #0xa]\n"
"    bl      sub_fc2825e2\n"
"    ldrsh.w r0, [r4, #0xc]\n"
"    movs    r1, #0\n"
"    bl      sub_fc282626\n"
"    mov     r5, r0\n"
"    ldr     r0, [r4]\n"
"    ldr.w   r8, =0x0005c18c\n"
"    cmp     r0, #0xc\n"
"    bne     loc_fc283672\n"
"    movs    r6, #0\n"
"    mov     r5, r6\n"
"    b       loc_fc28368a\n"
"loc_fc283672:\n"
"    cmp     r6, #1\n"
"    bne     loc_fc28368a\n"
"    ldrsh.w r0, [r4, #4]\n"
"    movs    r2, #2\n"
"    ldr     r1, =0xfc282353\n"
"    bl      sub_fc0d748a\n"
"    strh    r0, [r4, #4]\n"
"    movs    r0, #0\n"
"    str     r0, [r7, #0x28]\n"
"    b       loc_fc283690\n"
"loc_fc28368a:\n"
"    ldrh.w  r0, [r8]\n"
"    strh    r0, [r4, #4]\n"
"loc_fc283690:\n"
"    cmp     r5, #1\n"
"    bne     loc_fc2836a2\n"
"    ldrsh.w r0, [r4, #0xc]\n"
"    movs    r2, #0x20\n"
"    ldr     r1, =0xfc2823a7\n"
"    bl      sub_fc2874dc\n"
"    b       loc_fc2836a6\n"
"loc_fc2836a2:\n"
"    ldrh.w  r0, [r8, #8]\n"
"loc_fc2836a6:\n"
"    strh    r0, [r4, #0xc]\n"
"    ldrsh.w r0, [r4, #6]\n"
"    bl      sub_fc3a0c30_my\n" // ->
"    ldr     pc, =0xfc2836b1\n" // continue in firmware, thumb
/*
"    ldrsh.w r0, [r4, #8]\n"
"    movs    r1, #1\n"
"    bl      sub_fc13098e\n"
"    movs    r1, #0\n"
"    add.w   r0, r4, #8\n"
"    bl      sub_fc1309f6\n"
"    ldrsh.w r0, [r4, #0xe]\n"
"    bl      sub_fc0e47e8\n"
"    cmp     r6, #1\n"
"    movw    r4, #0xbb8\n"
"    bne     loc_fc2836f0\n"
"    ldr     r0, [r7, #0x1c]\n"
"    movs    r1, #2\n"
"    mov     r2, r4\n"
"    blx     sub_fc2c7cd0\n" // j_WaitForAllEventFlag
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc2836f0\n"
"    movw    r2, #0xa28\n"
"    ldr     r1, =0xfc282b80\n" //  *"ExpDrv.c"
"    movs    r0, #0\n"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc2836f0:\n"
"    cmp     r5, #1\n"
"    bne     loc_fc283714\n"
"    ldr     r0, [r7, #0x1c]\n"
"    movs    r1, #0x20\n"
"    mov     r2, r4\n"
"    blx     sub_fc2c7cd0\n" // j_WaitForAllEventFlag
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc283714\n"
"    movw    r2, #0xa2d\n"
"    ldr     r1, =0xfc282b80\n" //  *"ExpDrv.c"
"    pop.w   {r4, r5, r6, r7, r8, lr}\n"
"    movs    r0, #0\n"
"    b.w     loc_fc2c7314\n" // -> DebugAssert
"loc_fc283714:\n"
"    pop.w   {r4, r5, r6, r7, r8, pc}\n"
*/
".ltorg\n"
    );
}
// -f=chdk -s=0xfc3a0c31 -c=35
void __attribute__((naked,noinline)) sub_fc3a0c30_my() {
asm volatile (
"    push    {r4, r5, r6, lr}\n"
"    ldr     r5, =0x0000d218\n"
"    mov     r4, r0\n"
"    ldr     r0, [r5, #4]\n"
"    cmp     r0, #1\n"
"    beq     loc_fc3a0c48\n"
"    movs    r0, #0\n"
"    movw    r2, #0x16b\n"
"    ldr     r1, =0xfc3a0c90\n" //  *"Shutter.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc3a0c48:\n"
"    ldr     r0, =0xfffff400\n"
"    cmp     r4, r0\n"
"    bne     loc_fc3a0c52\n"
"    ldrsh.w r4, [r5, #2]\n"
"loc_fc3a0c52:\n"
"    strh    r4, [r5, #2]\n"
"    cmp     r4, r0\n"
"    bne     loc_fc3a0c64\n"
"    movs    r0, #0\n"
"    movw    r2, #0x171\n"
"    ldr     r1, =0xfc3a0c90\n" //  *"Shutter.c"
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc3a0c64:\n"
"    mov     r0, r4\n"
//"    bl      _apex2us\n"
"    bl      apex2us\n" // +
//"    mov     r4, r0\n"  // removed due to nullsub
//"    bl      sub_fc14a2a2\n" // nullsub
//"    mov     r0, r4\n"
"    bl      sub_fc14a830\n"
"    lsls    r0, r0, #0x1f\n"
"    beq     loc_fc3a0c8a\n"
"    pop.w   {r4, r5, r6, lr}\n"
"    movs    r0, #0\n"
"    movw    r2, #0x176\n"
"    ldr     r1, =0xfc3a0c90\n" //  *"Shutter.c"
//"    b.w     loc_fc2c7314\n" // -> DebugAssert
"    blx     sub_fc2c7de0\n" // j_DebugAssert
"loc_fc3a0c8a:\n"
"    pop     {r4, r5, r6, pc}\n"
".ltorg\n"
    );
}
