/*
 * boot.c - auto-generated by CHDK code_gen.
 */
#include "lolevel.h"
#include "platform.h"
#include "core.h"
#include "dryos31.h"

#define offsetof(TYPE, MEMBER) ((int) &((TYPE *)0)->MEMBER)

const char * const new_sa = &_end;

/*----------------------------------------------------------------------
    CreateTask_spytask
-----------------------------------------------------------------------*/
void CreateTask_spytask()
{
    _CreateTask("SpyTask", 0x19, 0x2000, core_spytask, 0);
}

/*----------------------------------------------------------------------
    boot()

    Main entry point for the CHDK code
-----------------------------------------------------------------------*/

/*************************************************************/
//** boot @ 0xFF82000C - 0xFF82017C, length=93
void __attribute__((naked,noinline)) boot() {
asm volatile (
"    LDR     R1, =0xC0410000 \n"
"    MOV     R0, #0 \n"
"    STR     R0, [R1] \n"
"    MOV     R1, #0x78 \n"
"    MCR     p15, 0, R1, c1, c0 \n"
"    MOV     R1, #0 \n"
"    MCR     p15, 0, R1, c7, c10, 4 \n"
"    MCR     p15, 0, R1, c7, c5 \n"
"    MCR     p15, 0, R1, c7, c6 \n"
"    MOV     R0, #0x3D \n"
"    MCR     p15, 0, R0, c6, c0 \n"
"    MOV     R0, #0xC000002F \n"
"    MCR     p15, 0, R0, c6, c1 \n"
"    MOV     R0, #0x35 \n"
"    MCR     p15, 0, R0, c6, c2 \n"
"    MOV     R0, #0x40000035 \n"
"    MCR     p15, 0, R0, c6, c3 \n"
"    MOV     R0, #0x80000017 \n"
"    MCR     p15, 0, R0, c6, c4 \n"
"    LDR     R0, =0xFF80002D \n"
"    MCR     p15, 0, R0, c6, c5 \n"
"    MOV     R0, #0x34 \n"
"    MCR     p15, 0, R0, c2, c0 \n"
"    MOV     R0, #0x34 \n"
"    MCR     p15, 0, R0, c2, c0, 1 \n"
"    MOV     R0, #0x34 \n"
"    MCR     p15, 0, R0, c3, c0 \n"
"    LDR     R0, =0x3333330 \n"
"    MCR     p15, 0, R0, c5, c0, 2 \n"
"    LDR     R0, =0x3333330 \n"
"    MCR     p15, 0, R0, c5, c0, 3 \n"
"    MRC     p15, 0, R0, c1, c0 \n"
"    ORR     R0, R0, #0x1000 \n"
"    ORR     R0, R0, #4 \n"
"    ORR     R0, R0, #1 \n"
"    MCR     p15, 0, R0, c1, c0 \n"
"    MOV     R1, #0x80000006 \n"
"    MCR     p15, 0, R1, c9, c1 \n"
"    MOV     R1, #6 \n"
"    MCR     p15, 0, R1, c9, c1, 1 \n"
"    MRC     p15, 0, R1, c1, c0 \n"
"    ORR     R1, R1, #0x50000 \n"
"    MCR     p15, 0, R1, c1, c0 \n"
"    LDR     R2, =0xC0200000 \n"
"    MOV     R1, #1 \n"
"    STR     R1, [R2, #0x10C] \n"
"    MVN     R1, #0 \n"
"    STR     R1, [R2, #0xC] \n"
"    STR     R1, [R2, #0x1C] \n"
"    STR     R1, [R2, #0x2C] \n"
"    STR     R1, [R2, #0x3C] \n"
"    STR     R1, [R2, #0x4C] \n"
"    STR     R1, [R2, #0x5C] \n"
"    STR     R1, [R2, #0x6C] \n"
"    STR     R1, [R2, #0x7C] \n"
"    STR     R1, [R2, #0x8C] \n"
"    STR     R1, [R2, #0x9C] \n"
"    STR     R1, [R2, #0xAC] \n"
"    STR     R1, [R2, #0xBC] \n"
"    STR     R1, [R2, #0xCC] \n"
"    STR     R1, [R2, #0xDC] \n"
"    STR     R1, [R2, #0xEC] \n"
"    STR     R1, [R2, #0xFC] \n"
"    LDR     R1, =0xC0400008 \n"
"    LDR     R2, =0x430005 \n"
"    STR     R2, [R1] \n"
"    MOV     R1, #1 \n"
"    LDR     R2, =0xC0243100 \n"
"    STR     R2, [R1] \n"
"    LDR     R2, =0xC0242010 \n"
"    LDR     R1, [R2] \n"
"    ORR     R1, R1, #1 \n"
"    STR     R1, [R2] \n"
"    LDR     R0, =0xFFCC8F04 \n"
"    LDR     R1, =0x6B1000 \n"
"    LDR     R3, =0x6C1402 \n"

"loc_FF82013C:\n"
"    CMP     R1, R3 \n"
"    LDRCC   R2, [R0], #4 \n"
"    STRCC   R2, [R1], #4 \n"
"    BCC     loc_FF82013C \n"
"    LDR     R0, =0xFFCBB0E4 \n"
"    LDR     R1, =0x1900 \n"
"    LDR     R3, =0xF720 \n"

"loc_FF820158:\n"
"    CMP     R1, R3 \n"
"    LDRCC   R2, [R0], #4 \n"
"    STRCC   R2, [R1], #4 \n"
"    BCC     loc_FF820158 \n"
"    LDR     R1, =0x18EB58 \n"
"    MOV     R2, #0 \n"

"loc_FF820170:\n"
"    CMP     R3, R1 \n"
"    STRCC   R2, [R3], #4 \n"
"    BCC     loc_FF820170 \n"
/* Install task hooks via 0x193x is not possible with this new dryos version
   So the below code patches the CreateTask function in RAM to install our
   hook -- ERR99
*/
// Install CreateTask patch
"    LDR     R0, =patch_CreateTask\n"   // Patch data
"    LDM     R0, {R1,R2}\n"             // Get two patch instructions
"    LDR     R0, =hook_CreateTask\n"    // Address to patch
"    STM     R0, {R1,R2}\n"             // Store patch instructions

"    B       sub_FF8203C4_my \n"  // --> Patched. Old value = 0xFF8203C4.

"patch_CreateTask:\n"
"    LDR     PC, [PC,#-0x4]\n"          // Do jump to absolute address CreateTask_my
"    .long   CreateTask_my\n"
);
}

/*************************************************************/
//** CreateTask_my @ 0x006B562C - 0x006B5630, length=2
void __attribute__((naked,noinline)) CreateTask_my() {
asm volatile (
"    STMFD   SP!, {R0}\n"
//R3 = Pointer to task function to create

/*** INSTALL capt_seq_task() hook ***/
"    LDR     R0, =task_CaptSeq\n"       // DryOS original code function ptr.
"    CMP     R0, R3\n"                  // is the given taskptr equal to our searched function?
"    LDREQ   R3, =capt_seq_task\n"      // if so replace with our task function base ptr.
"    BEQ     exitHook\n"                // below compares not necessary if this check has found something.

/*** INSTALL exp_drv_task() hook ***/
"    LDR     R0, =task_ExpDrv\n"
"    CMP     R0, R3\n"
"    LDREQ   R3, =exp_drv_task\n"
"    BEQ     exitHook\n"

/*** INSTALL filewrite() hook ***/
"    LDR     R0, =task_FileWrite\n"
"    CMP     R0, R3\n"
"    LDREQ   R3, =filewritetask\n"
"    BEQ     exitHook\n"

/*** INSTALL movie_record_task() hook ***/
/*
"    LDR     R0, =task_MovieRecord\n"
"    CMP     R0, R3\n"
"    LDREQ   R3, =movie_record_task\n"
"    BEQ     exitHook\n"
*/

/*** INSTALL init_file_modules_task() hook ***/
"    LDR     R0, =task_InitFileModules\n"
"    CMP     R0, R3\n"
"    LDREQ   R3, =init_file_modules_task\n"

"exitHook:\n"
// restore overwritten registers
"    LDMFD   SP!, {R0}\n"
// Execute overwritten instructions from original code, then jump to firmware
"    STMFD   SP!, {R1-R9,LR} \n"
"    MOV     R4, R0 \n"
"    LDR     PC, =0x006B5634 \n"  // Continue in firmware
);
}

/*************************************************************/
//** sub_FF8203C4_my @ 0xFF8203C4 - 0xFF82042C, length=27
void __attribute__((naked,noinline)) sub_FF8203C4_my() {

    //Replacement of sub_ for correct power-on.
    //(short press = playback mode, long press = record mode)
    // Similar to ixus160: http://chdk.setepontos.com/index.php?topic=12321.msg123724#msg123724
    // look at power-on switch sub_ff868644 (ff866334) => (same as from ixus145) 0xC022F48C
    // value and pointer from sub_ff868098 (0x29C0)
    *(int*)(0x29C0+0x4) = (*(int*)0xC022F48C)&0x100000 ? 0x200000 : 0x100000;
asm volatile (
"    LDR     R0, =0xFF82043C \n"
"    MOV     R1, #0 \n"
"    LDR     R3, =0xFF820474 \n"

"loc_FF8203D0:\n"
"    CMP     R0, R3 \n"
"    LDRCC   R2, [R0], #4 \n"
"    STRCC   R2, [R1], #4 \n"
"    BCC     loc_FF8203D0 \n"
"    LDR     R0, =0xFF820474 \n"
"    MOV     R1, #0x1C0 \n"
"    LDR     R3, =0xFF82065C \n"

"loc_FF8203EC:\n"
"    CMP     R0, R3 \n"
"    LDRCC   R2, [R0], #4 \n"
"    STRCC   R2, [R1], #4 \n"
"    BCC     loc_FF8203EC \n"
"    MOV     R0, #0xD2 \n"
"    MSR     CPSR_cxsf, R0 \n"
"    MOV     SP, #0x1000 \n"
"    MOV     R0, #0xD3 \n"
"    MSR     CPSR_cxsf, R0 \n"
"    MOV     SP, #0x1000 \n"
"    LDR     R0, =0x3A8 \n"
"    LDR     R2, =0xEEEEEEEE \n"
"    MOV     R3, #0x1000 \n"

"loc_FF820420:\n"
"    CMP     R0, R3 \n"
"    STRCC   R2, [R0], #4 \n"
"    BCC     loc_FF820420 \n"
"    B       sub_FF8228DC_my \n"  // --> Patched. Old value = 0xFF8228DC.
);
}

/*************************************************************/
//** sub_FF8228DC_my @ 0xFF8228DC - 0xFF822A50, length=94
void __attribute__((naked,noinline)) sub_FF8228DC_my() {
asm volatile (
"    LDR     R0, =0x1A58 \n"
"    STR     LR, [SP, #-4]! \n"
"    SUB     SP, SP, #0x7C \n"
"    MOV     R1, #0x80000 \n"
"    STR     R1, [R0] \n"
"    LDR     R0, =0x40BCE980 \n"
"    LDR     R1, =0x1A5C \n"
"    STR     R0, [R1] \n"
"    LDR     R1, =0x1A60 \n"
"    ADD     R0, R0, #0x2000 \n"
"    STR     R0, [R1] \n"
"    MOV     R1, #0x78 \n"
"    ADD     R0, SP, #4 \n"
"    BL      sub_006BD068 /*_bzero*/ \n"
"    MOV     R0, #0x84000 \n"
"    STR     R0, [SP, #8] \n"

#if defined(CHDK_NOT_IN_CANON_HEAP) // use original heap offset if CHDK is loaded in high memory
"    LDR     R0, =0x18EB58 \n"
#else
"    LDR     R0, =new_sa\n"   // otherwise use patched value
"    LDR     R0, [R0]\n"      //
#endif

"    LDR     R2, =0x5817B0 \n"
"    STR     R0, [SP, #0xC] \n"
"    SUB     R0, R2, R0 \n"
"    STR     R0, [SP, #0x10] \n"
"    MOV     R0, #0x22 \n"
"    STR     R0, [SP, #0x1C] \n"
"    MOV     R0, #0x7C \n"
"    STR     R0, [SP, #0x20] \n"
"    LDR     R1, =0x58A000 \n"
"    LDR     R0, =0x1D3 \n"
"    STR     R1, [SP, #4] \n"
"    STR     R0, [SP, #0x24] \n"
"    MOV     R0, #0x96 \n"
"    STR     R2, [SP, #0x14] \n"
"    STR     R1, [SP, #0x18] \n"
"    STR     R0, [SP, #0x28] \n"
"    STR     R0, [SP, #0x2C] \n"
"    MOV     R0, #0x64 \n"
"    STR     R0, [SP, #0x30] \n"
"    MOV     R0, #0 \n"
"    STR     R0, [SP, #0x34] \n"
"    STR     R0, [SP, #0x38] \n"
"    STR     R0, [SP, #0x3C] \n"
"    MOV     R0, #0x10 \n"
"    STR     R0, [SP, #0x60] \n"
"    MOV     R0, #0x1000 \n"
"    STR     R0, [SP, #0x64] \n"
"    MOV     R0, #0x100 \n"
"    STR     R0, [SP, #0x68] \n"
"    MOV     R0, #0x2000 \n"
"    STR     R0, [SP, #0x6C] \n"
"    LDR     R1, =sub_FF8248DC_my \n"  // --> Patched. Old value = 0xFF8248DC.
"    MOV     R2, #0 \n"
"    ADD     R0, SP, #4 \n"
"    BL      sub_006B1B9C \n"
"    ADD     SP, SP, #0x7C \n"
"    LDR     PC, [SP], #4 \n"
);
}

/*************************************************************/
//** sub_FF8248DC_my @ 0xFF8248DC - 0xFF824974, length=39
void __attribute__((naked,noinline)) sub_FF8248DC_my() {
asm volatile (
"    STMFD   SP!, {R4,LR} \n"
"    LDR     R4, =0xFF824998 /*'/_term'*/ \n"
"    BL      sub_FF820858 \n"
"    LDR     R1, =0x1A58 \n"
"    LDR     R0, =0x19F0 \n"
"    LDR     R1, [R1] \n"
"    LDR     R0, [R0] \n"
"    ADD     R1, R1, #0x10 \n"
"    CMP     R0, R1 \n"
"    LDRCC   R0, =0xFF8249A8 /*'USER_MEM size checking'*/ \n"
"    BLCC    _err_init_task \n"
"    BL      sub_FF8236A4 \n"
"    CMP     R0, #0 \n"
"    LDRLT   R0, =0xFF8249C0 /*'dmSetup'*/ \n"
"    BLLT    _err_init_task \n"
"    BL      sub_FF8224E8 \n"
"    CMP     R0, #0 \n"
"    LDRLT   R0, =0xFF8249C8 /*'termDriverInit'*/ \n"
"    BLLT    _err_init_task \n"
"    MOV     R0, R4 \n"
"    BL      sub_FF8225D0 \n"
"    CMP     R0, #0 \n"
"    LDRLT   R0, =0xFF8249D8 /*'termDeviceCreate'*/ \n"
"    BLLT    _err_init_task \n"
"    MOV     R0, R4 \n"
"    BL      sub_FF822100 \n"
"    CMP     R0, #0 \n"
"    LDRLT   R0, =0xFF8249EC /*'stdioSetup'*/ \n"
"    BLLT    _err_init_task \n"
"    BL      sub_FF824F44 \n"
"    CMP     R0, #0 \n"
"    LDRLT   R0, =0xFF8249F8 /*'stdlibSetup'*/ \n"
"    BLLT    _err_init_task \n"
"    BL      sub_FF8210A8 \n"
"    CMP     R0, #0 \n"
"    LDRLT   R0, =0xFF824A04 /*'extlib_setup'*/ \n"
"    BLLT    _err_init_task \n"
"    LDMFD   SP!, {R4,LR} \n"
"    B       sub_FF827C18_my \n"  // --> Patched. Old value = 0xFF827C18.
);
}

/*************************************************************/
//** sub_FF827C18_my @ 0xFF827C18 - 0xFF827C88, length=29
void __attribute__((naked,noinline)) sub_FF827C18_my() {
asm volatile (
"    STMFD   SP!, {R3,LR} \n"
//"  BL      _sub_FF896400 \n"  // --> Nullsub call removed.
"    BL      sub_FF83672C \n"
"    CMP     R0, #0 \n"
"    BNE     loc_FF827C3C \n"
"    BL      sub_FF82C900 /*_IsNormalCameraMode_FW*/ \n"
"    CMP     R0, #0 \n"
"    MOVNE   R0, #1 \n"
"    BNE     loc_FF827C40 \n"

"loc_FF827C3C:\n"
"    MOV     R0, #0 \n"

"loc_FF827C40:\n"
//"  BL      _sub_FF82BA84 \n"  // See: http://chdk.setepontos.com/index.php?topic=12321.msg121619#msg121619
//"  CMP     R0, #0 \n"
//"  BNE     loc_FF827C54 \n"
//"  BL      _sub_FF82B26C \n"

"loc_FF827C50:\n"
//"  B       loc_FF827C50 \n"

"loc_FF827C54:\n"
"    BL      sub_006B8A8C \n"
"    LDR     R1, =0x60E000 \n"
"    MOV     R0, #0 \n"
"    BL      sub_FF836C24 \n"
"    BL      sub_006B8DD4 /*_EnableDispatch*/ \n"
"    MOV     R3, #0 \n"
"    STR     R3, [SP] \n"
"    LDR     R3, =task_Startup_my \n"  // --> Patched. Old value = 0xFF827BB0.
"    MOV     R2, #0 \n"
"    MOV     R1, #0x19 \n"
"    LDR     R0, =0xFF827C94 /*'Startup'*/ \n"
"    BL      _CreateTask \n"
"    MOV     R0, #0 \n"
"    LDMFD   SP!, {R3,PC} \n"
);
}

/*************************************************************/
//** task_Startup_my @ 0xFF827BB0 - 0xFF827C14, length=26
void __attribute__((naked,noinline)) task_Startup_my() {
asm volatile (
"    STMFD   SP!, {R4,LR} \n"
"    BL      sub_FF822E10 \n"
"    BL      sub_FF82BED8 \n"
"    BL      sub_FF82B218 \n"
//"  BL      _sub_FF868654 \n"  // --> Nullsub call removed.
"    BL      sub_FF836968 \n"
//"  BL      _sub_FF836828 \n"  // load DISKBOOT.BIN
"    BL      sub_FF836AC0 \n"
"    BL      sub_FF836CF8 \n"
//"  BL      _sub_FF836AB4 \n"  // --> Nullsub call removed.
"    BL      sub_FF8369A4 \n"
"    BL      sub_FF8329C4 \n"
"    BL      sub_FF836D00 \n"
"    BL      CreateTask_spytask\n" // added
"    BL      taskcreatePhySw_my \n"  // --> Patched. Old value = 0xFF82B930.
"    BL      init_required_fw_features\n" // added
"    BL      sub_FF830AE8 \n"
"    BL      sub_FF8ACBD4 \n"
"    BL      sub_FF829008 \n"
"    BL      sub_FF82AD74 \n"
"    BL      sub_FF83653C \n"
"    BL      sub_FF82B1CC \n"
"    BL      sub_FF82AD08 \n"
//"  BL      _sub_FF836AB8 \n"  // --> Nullsub call removed.
"    BL      sub_FF829C98 \n"
"    BL      sub_FF82ACC4 \n"
"    LDMFD   SP!, {R4,LR} \n"
"    B       sub_FF822F38 \n"
);
}

/*************************************************************/
//** taskcreatePhySw_my @ 0xFF82B930 - 0xFF82B974, length=18
void __attribute__((naked,noinline)) taskcreatePhySw_my() {
asm volatile (
"    STMFD   SP!, {R3-R5,LR} \n"
"    BL      sub_FF83575C \n"
"    BL      sub_FF82C838 /*_IsFactoryMode_FW*/ \n"
"    CMP     R0, #0 \n"
"    BLEQ    sub_FF8356C4 /*_OpLog.Start_FW*/ \n"
"    LDR     R4, =0x1C94 \n"
"    LDR     R0, [R4, #4] \n"
"    CMP     R0, #0 \n"
"    BNE     loc_FF82B974 \n"
"    MOV     R3, #0 \n"
"    STR     R3, [SP] \n"
"    LDR     R3, =mykbd_task \n"  // --> Patched. Old value = 0xFF82B8FC.
"    MOV     R2, #0x2000 \n"  // --> Patched. Old value = 0x800. stack size for new task_PhySw so we don't have to do stack switch
"    MOV     R1, #0x17 \n"
"    LDR     R0, =0xFF82BBE0 /*'PhySw'*/ \n"
"    BL      sub_006B7B9C /*_CreateTaskStrictly*/ \n"
"    STR     R0, [R4, #4] \n"

"loc_FF82B974:\n"
"    LDMFD   SP!, {R3-R5,PC} \n"
);
}

/*************************************************************/
//** init_file_modules_task @ 0xFF857BD0 - 0xFF857C04, length=14
void __attribute__((naked,noinline)) init_file_modules_task() {
asm volatile (
"    STMFD   SP!, {R4-R6,LR} \n"
"    BL      sub_FF89D02C \n"
"    LDR     R5, =0x5006 \n"
"    MOVS    R4, R0 \n"
"    MOVNE   R1, #0 \n"
"    MOVNE   R0, R5 \n"
"    BLNE    _PostLogicalEventToUI \n"
"    BL      sub_FF89D060 \n"
"    BL      core_spytask_can_start\n"  // CHDK: Set "it's-safe-to-start" flag for spytask
"    CMP     R4, #0 \n"
"    LDMNEFD SP!, {R4-R6,PC} \n"
"    MOV     R0, R5 \n"
"    LDMFD   SP!, {R4-R6,LR} \n"
"    MOV     R1, #0 \n"
"    B       _PostLogicalEventToUI \n"
);
}

/*
    *** TEMPORARY workaround ***
    Init stuff to avoid asserts on cameras running DryOS r54+
    Execute this only once
 */
void init_required_fw_features(void) {
    extern void _init_focus_eventflag();
    extern void _init_nd_eventflag();
    extern void _init_nd_semaphore();
    //extern void _init_zoom_semaphore(); // for MoveZoomLensWithPoint

    _init_focus_eventflag();
    _init_nd_eventflag();
    _init_nd_semaphore();
}