...
int *conf_reader_pos;
int *conf_reader_wrap_by_words;
color *conf_reader_color;
char *conf_reader_file;
...
static void read_goto_next_line() {
    draw_filled_rect(xx, yy, x+w-1, yy+rbf_font_height()-1, MAKE_COLOR(*conf_reader_color>>8, *conf_reader_color>>8));
    xx  = x;
    yy += rbf_font_height();
}
...
void gui_read_draw(int enforce_redraw) {
    if (*conf_reader_autoscroll && !pause && get_tick_count()-last_time >= *conf_reader_autoscroll_delay*1000 && (*conf_reader_pos+read_on_screen)<read_file_size) {
        *conf_reader_pos += read_on_screen;
        read_to_draw = 1;
    }
    if (read_to_draw) {
        ...
        lseek(read_file, *conf_reader_pos, SEEK_SET);
        read_on_screen=0;
        ...
        while (yy<=y+h-rbf_font_height()) {
            ...
            if (n==0) {
                 read_goto_next_line();
                 if (yy < y+h)
                     draw_filled_rect(x, yy, x+w-1, y+h-1, MAKE_COLOR(*conf_reader_color>>8, *conf_reader_color>>8));
                 break;
            }
            i=0;
            while (i<n && yy<=y+h-rbf_font_height()) {
                switch (buffer[i]) {
                    ...
                    default:
                        ...
                        xx+=rbf_draw_char(xx, yy, buffer[i], *conf_reader_color);
                        break;
                }
                ++i;
                if (xx >= x+w) {
        ...
        if (read_file_size) {
            ...
            if (n<20) n=20;
            i=(i-n)**conf_reader_pos/read_file_size;   // top pos
...
void gui_read_kbd_process() {
    switch (kbd_get_autoclicked_key() | get_jogdial_direction()) {
        ...
        case KEY_UP:
        case KEY_LEFT:
            if (*conf_reader_pos>0) {
                *conf_reader_pos -= 45*15;
                if (*conf_reader_pos<0) *conf_reader_pos=0;
                read_to_draw = 1;
            }
            break;
        case JOGDIAL_RIGHT:
        ...
        case KEY_RIGHT:
        case KEY_SHOOT_HALF:
            if ((*conf_reader_pos+read_on_screen)<read_file_size) {
                *conf_reader_pos += read_on_screen;
                read_to_draw = 1;
            }
            break;
...
int _module_loader( void** chdk_export_list )
{
  ...
  CONF_BIND_STR( 38, conf_reader_file             );
  CONF_BIND_INT( 39, conf_reader_pos              );
  CONF_BIND_INT( 43, conf_reader_autoscroll       );
  CONF_BIND_INT( 44, conf_reader_autoscroll_delay );
  CONF_BIND_INT( 61, conf_reader_wrap_by_words    );
  CONF_BIND_STR( 66, conf_menu_rbf_file           );
