...
// Zebra for cameras with YUV bitmap buffer - works; but not efficient and flickers badly
...
static int draw_zebra(int mrec, unsigned int f)
{
    ...
    viewport_width = vid_get_viewport_width_proper();
...
int gui_osd_draw_zebra(int show)
{
    ...
    return draw_zebra(camera_info.state.mode_rec,f);
}
