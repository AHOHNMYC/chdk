...
#define FIELD_COLOR	 MAKE_COLOR(COLOR_GREY_LT,COLOR_GREY_LT)//(f�llfarbe,rand)
...
char ki_3(char player) {
  int i,j;
  //waagerecht und Diagonal (�ber Anstieg)
...
char ki_2(char player) {
  ...
  for(i=0;i<=3;i++){		//column
    for(j=1;j<=6;j++){		//row 
      if(field[i][j]==player&&field[i+1][j]==player&&field[i+2][j]==0&&field[i+3][j]==0&&(field[i+2][j-1]||field[i+3][j-1])&&!ki_isBadColumn(i+3)) return i+3;	//**-- return und if �ndern
    ...
    for(j=1;j<=3;j++){
      if(field[i][j]==player&&field[i+1][j+1]==player&&field[i+2][j+2]==0&&field[i+3][j+3]==0&&field[i+2][j+2-1]&&!ki_isBadColumn(i+3)) return i+3;	//**-- return und if �ndern
      if(field[i][j]==player&&field[i+1][j+1]==player&&field[i+2][j+2]==0&&field[i+3][j+3]==0&&field[i+3][j+3-1]&&!ki_isBadColumn(i+3)) return i+4;	//**-- return und if �ndern
...
char ki_findColumn(char mode, char player) {							//player = 1|2
  ...
  case 1: erg=ki_3(cam);												//ich3
          ...
          if(!erg) {													//ung�nstige Spalten ermitteln
            for(i=1;i<=7;i++) {
              if(!isFull(i)) {
                set_stone(i,9,0);										//Stein setzen 9=irgendwas
                badColumns[i-1]=ki_findColumn('b',otherplayer);			//ung�nstige Spalten ermitteln b=bad
                if(!badColumns[i-1]) badColumns[i-1]=ki_findColumn('b',cam);
                unset_stone(i);
              } else badColumns[i-1]=9;								//9=full
            }
          } else break;
          erg=ki_2(cam);												//ich2 (f�r passivere Methode vertauschen)
